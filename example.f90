...
module parameters
    ...
    double precision, parameter :: bunsi3 = 195.084D-3 ! 分子の質量  kg/mol
    double precision, parameter :: sig33 = 2.54D0 !分子のLennard-Jonesパラメータσ(無次元)
    double precision, parameter :: eps33 = 83.31D-5 !分子のLennard-Jonesパラメータε(無次元)
...
contains
	subroutine seting
        ...
        !write(6,*) zmass, sig, eps, xsyul, xcutof, cforce
	...
	subroutine record
		use variable, only: posx, posy, posz, velx, vely, velz, forx, fory, forz
		use parameters
		implicit none
		integer :: i
		do i = 1, nkoss
			write(3, '(I6, 3E15.7)') i, posx(i), posy(i), posz(i)
			write(4, '(I6, 3E15.7)') i, velx(i), vely(i), velz(i)
            write(13, '(i6, 3E15.7)') i, forx(i), fory(i), forz(i)
		end do
	...
	subroutine record3
        ...
        integer :: i
        do i = 1, nkoss
            write(9, '(I6, 6E15.7)') i, posx(i), posy(i), posz(i), velx(i), vely(i), velz(i)
        end do
