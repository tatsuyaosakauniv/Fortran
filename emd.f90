...
!--------------- よく変更する---------------------!
    ...
    integer, parameter :: numz(TYPMOL) = [ 5,15, 5]
...
module variable
    ...
    ! Green-Kubo用
    double precision :: ensembleHeat(plot+10,TYPMOL) ! 熱流束の内積を格納する配列
...
program main
    ...
    do i = 1, stpMax
        ...
        if (mod(stpNow,500) == 0) then
            write(6,'(3X, I7, 1X, A3, I9)') stpNow, ' / ', stpMax
        endif
        ...
        if(stpNow > stpScaling + stpRelax) then
            do j = 1, TYPMOL
                ...
                do i = 1, nummol(j) ! Pt分子全体
                    nowHeat(i) = (interForce(i,1,j)*1.000d-6 * typ(j)%mol(i)%vtmp(1)*1.000d+5 + &
    ...
    subroutine record_final ! 最終状態を記録
        ...
        do j = 1, TYPMOL
            ...
            do i = 1, plot
                ensembleHeat(i,j) = ensembleHeat(i,j) / numEnsemble
                write(81, *) i*timeInterval, ensembleHeat(i,1), ensembleHeat(i,3)
            end do
