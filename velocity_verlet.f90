...
contains
	...
	subroutine calcu
        ...
        do i=1, nkoss
          vtempx(i) = velx(i) + 0.500D0*forx(i)*dt;
          vtempy(i) = vely(i) + 0.500D0*fory(i)*dt;
          vtempz(i) = velz(i) + 0.500D0*forz(i)*dt;
          posx(i) = posx(i) + vtempx(i)*dt;
          posy(i) = posy(i) + vtempy(i)*dt;
          posz(i) = posz(i) + vtempz(i)*dt;
		end do
		...
		do i1= 1, nkoss
        	do i2= i1+1, nkoss
				...
				forcey = -force*divy/zmass
				forcez = -force*divz/zmass
				forx2(i1) = forx2(i1) + forcex
				forx2(i2) = forx2(i2) - forcex
				fory2(i1) = fory2(i1) + forcey
				fory2(i2) = fory2(i2) - forcey
				forz2(i1) = forz2(i1) + forcez
				forz2(i2) = forz2(i2) - forcez
			end do
		end do

		do i=1, nkoss
			velx(i) = velx(i) + 0.500D0*(forx(i)+forx2(i))*dt;
			vely(i) = vely(i) + 0.500D0*(fory(i)+fory2(i))*dt;
			velz(i) = velz(i) + 0.500D0*(forz(i)+forz2(i))*dt;
		end do
